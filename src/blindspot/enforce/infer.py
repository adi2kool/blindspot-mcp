...
Config via environment (mirrors the judge):
  BLINDSPOT_INFER        on | off | auto   (default auto)
  BLINDSPOT_INFER_MODEL  model tag          (default llama3.2)
  BLINDSPOT_INFER_URL    base url           (default http://localhost:11434)
"""
...
class ProvenanceInferer:
    ...
    def __init__(
        ...
        self.mode = (mode or os.environ.get("BLINDSPOT_INFER", "auto")).lower()
        self.model = model or os.environ.get("BLINDSPOT_INFER_MODEL", "llama3.2")
        self.base_url = (
            base_url or os.environ.get("BLINDSPOT_INFER_URL", "http://localhost:11434")
        ).rstrip("/")

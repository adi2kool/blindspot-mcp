...
Like `package-lock.json` plus a signing requirement, for MCP. `blindspot lock` captures a
server's current surface (its tool / prompt / resource definitions) and its hash into a
`blindspot.lock`. The proxy, run with `--lock`, refuses to front a server whose surface
...
def load_lock(path: str | Path) -> dict:
    ...
    if not isinstance(data, dict) or "surface_hash" not in data:
        raise ValueError("not a valid blindspot.lock (missing surface_hash)")
    return data
